...
class TrackerGameContext(CommonContext):
    ...
    def on_package(self, cmd: str, args: dict):
        try:
            if cmd == 'Connected':
                ...
                try:
                    self.tracker_core = TrackerCore.find_tracker(connected_cls, args["slot_data"])
    ...
    def _set_host_settings(self):
        ...
        self.output_format, self.hide_excluded, self.use_split = report_type, tracker_settings[
    ...
    def run_generator(self, slot_data: dict | None = None, override_yaml_path: str | None = None):
        self.tracker_core.run_generator(slot_data=slot_data, override_yaml_path=override_yaml_path)
...
def get_logical_path(ctx: TrackerGameContext, dest_name: str):
    return ctx.tracker_core.get_logical_path(ctx, dest_name)


def updateTracker(ctx: TrackerGameContext) -> CurrentTrackerState:
    return ctx.tracker_core.updateTracker(ctx)
