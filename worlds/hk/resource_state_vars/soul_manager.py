...
class SoulManager(metaclass=ResourceStateHandler):
    ...
    def spend_soul(self, state_blob: Counter, item_state, amount: int) -> Generator[Counter]:
        ret = state_blob.copy()
    ...
    def try_spend_soul(self, state_blob: Counter, item_state, amount: int) -> bool:
    ...
    def spend_and_rebalance(self, state_blob: Counter, item_state, amount: int, soul: SoulInfo) -> None:
        self.spend_without_rebalance(state_blob, item_state, amount, soul)
        self.rebalance_reserve(state_blob, item_state, soul)

    def spend_without_rebalance(self, state_blob: Counter, item_state, amount: int, soul: SoulInfo) -> None:
        state_blob["SPENTSOUL"] += amount
        state_blob["REQUIREDMAXSOUL"] = max(state_blob["REQUIREDMAXSOUL"], state_blob["SPENTSOUL"])
        soul.soul -= amount

    def rebalance_reserve(self, state_blob: Counter, item_state, soul: SoulInfo) -> None:
    ...
    def try_spend_soul_sequence(self, state_blob: Counter, item_state, amount: int, casts: list[int]) -> bool:
        soul = self.get_soul_info(state_blob, item_state)
        for cast in casts:
    ...
    def spend_all_soul(self, state_blob: Counter, item_state) -> Generator[Counter]:
        ret = state_blob.copy()
        if self.try_spend_all_soul(ret, item_state):
            yield ret

    def try_spend_all_soul(self, state_blob: Counter, item_state) -> bool:
    ...
    def restore_all_soul(self, state_blob: Counter, item_state, restore_reserves: bool) -> Generator[Counter]:
        ret = state_blob.copy()
    ...
    def try_restore_all_soul(self, state_blob: Counter, item_state, restore_reserves: bool) -> bool:
    ...
    def try_set_soul_limit(self, state_blob: Counter, item_state, limiter: int, applies_to_prior_path: bool):
    ...
    def limit_soul(self, state_blob: Counter, item_state, limiter: int, applies_to_prior_path: bool):
        ret = state_blob.copy()
