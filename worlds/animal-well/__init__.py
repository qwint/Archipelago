...
class AnimalWellWorld(World):
    """
    Explore a land filled with lost legends, ancient powers, and ferocious monsters in ANIMAL WELL, a 2d action game
    about a small potato on a big adventure. Stranded on a mysterious beach, armed with only your own curiosity, you will
    confront colossal beasts, collect strange and powerful items, and unravel long-lost secrets. Be brave, tiny potato!
    """
    game = "ANIMAL WELL"
    web = AnimalWellWeb()
    ...
    def create_items(self) -> None:
        ...
        tunic_items: List[AnimalWellItem] = []
        ...
        if self.options.maskless:
            mask_item = AnimalWellItem("Scavenger Mask", ItemClassification.useful, self.item_name_to_id["Scavenger Mask"], self.player)
            tunic_items.append(mask_item)
            items_to_create["Scavenger Mask"] = 0

        if self.options.lanternless:
            lantern_item = AnimalWellItem("Lantern", ItemClassification.useful, self.item_name_to_id["Lantern"], self.player)
            tunic_items.append(lantern_item)
            items_to_create["Lantern"] = 0

        for item, quantity in items_to_create.items():
            for i in range(0, quantity):
                tunic_item: AnimalWellItem = self.create_item(item)
    ...
    def create_regions(self) -> None:
        ...
        if self.options.entrance_rando or self.options.shuffle_ladders:
            ...
            for location_name, location_id in self.location_name_to_id.items():
                ...
                location = AnimalWellLocation(self.player, location_name, location_id, region)
                region.locations.append(location)

            victory_region = self.multiworld.get_region("Spirit Arena", self.player)
            victory_location = AnimalWellLocation(self.player, "The Heir", None, victory_region)
            victory_location.place_locked_item(AnimalWellItem("Victory", ItemClassification.progression, None, self.player))
