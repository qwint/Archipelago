...
class AWData(NamedTuple):
    ...
    eggs_required: Optional[int] = None
...
traversal_requirements: Dict[str, Dict[str, AWData]] = {
    ...
    rname.starting_after_ghost: {
        rname.bird_area:
            AWData(AWType.region),
        lname.candle_first:
            AWData(AWType.location, match_required=True, event=iname.event_candle_first),
    ...
    rname.fish_west: {
        rname.fish_wand_pit:  # todo: check if you can use disc or something else
            AWData(AWType.region, [[iname.bubble]]),
    ...
    rname.bear_slink_room: {
        lname.slink_chest:
            AWData(AWType.location),
        # TODO: too tight to not be disc_hop_hard? It's close.
    ...
    rname.bear_connector_passage: {
        ...
        rname.bear_middle_phone_room:
            AWData(AWType.region),
        # you open some doors starting at the connector and ending at the door to here. Or, bubble over the wall
        # through the miasma. Will we call this bubble_short?
        rname.bear_match_chest_spot:
    ...
    rname.dog_area: {
        ...
        lname.candle_dog_dark:
            AWData(AWType.location, match_required=True, event=iname.event_candle_dog_dark),
    ...
    rname.dog_upper: {
        ...
        rname.dog_upper_above_switch_lines:  # todo: double check you can use remote in here
    ...
    rname.dog_upper_past_lake: {
        lname.candle_dog_switch_box:
            AWData(AWType.location, match_required=True, event=iname.event_candle_dog_switch_box),
    ...
    rname.dog_swordfish_lake_ledge: {
        ...
        lname.bunny_disc_spike:  # not disc hop since you literally need to do this
            AWData(AWType.location, [[iname.disc], [iname.bubble_long, iname.wheel_hop, iname.weird_skips]]),
    ...
    rname.dog_many_switches: {
        lname.candle_dog_many_switches:
            AWData(AWType.location, match_required=True, event=iname.event_candle_dog_many_switches),
        rname.dog_upside_down_egg_spot:
            AWData(AWType.region, [[iname.remote]]),
        rname.dog_bat_room:
            AWData(AWType.region),
    ...
    rname.dog_bat_room: {
        lname.candle_dog_bat:
            AWData(AWType.location, match_required=True, event=iname.event_candle_dog_bat),
        lname.key_dog:
            AWData(AWType.location),
    ...
    rname.dog_upper_above_switch_lines: {
        ...
        lname.candle_dog_disc_switches:
            AWData(AWType.location, [[iname.disc], [iname.remote], [iname.top]], match_required=True,
    ...
    rname.frog_near_wombat: {
        lname.candle_frog:
            AWData(AWType.location, match_required=True, event=iname.event_candle_frog),
    ...
    rname.frog_bird_after_yoyo_1: {
        ...
        rname.frog_worm_shaft_bottom:  # if you fall along the left side, the bird doesn't reach you in time
            AWData(AWType.region, [[iname.weird_skips], [iname.lantern]]),
        lname.egg_sapphire:
