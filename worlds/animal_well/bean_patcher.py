...
class Bean_Patcher:
    ...
    def apply_patches(self):
        ...
        address = self.application_state_address
        active_slot = self.process.read_bytes(address + 0x40C, 1)[0]
        slot_address = address + 0x400 + HEADER_LENGTH + (SAVE_SLOT_LENGTH * active_slot)
        ...
        self.custom_memory_space = self.process.allocate(custom_space_size)
        current_custom_space_offset = self.custom_memory_space

        if self.log_debug_info: self.log_info(f'Custom memory space: {hex(self.custom_memory_space)}, length of {hex(custom_space_size)}')
        ...
        self.main_menu_draw_string_addr = current_custom_space_offset
        current_custom_space_offset += len(title_screen_text)
        ...
        self.game_draw_routine_string_addr = current_custom_space_offset
        current_custom_space_offset += self.draw_routine_string_size + 0x10

        # self.game_draw_symbol_x_address = current_custom_space_offset
        # current_custom_space_offset += 4
        # self.game_draw_symbol_y_address = current_custom_space_offset
        # current_custom_space_offset += 4
        ...
        # region Room Palette Override Patch
        """
        Forces every room to use the specified palette. Adds a little extra visual variety to randomizer runs.
        """
        room_palette_override_patch_enabled = False
        if room_palette_override_patch_enabled:
            room_palette_override_shader = 0x14
            if self.log_debug_info: self.log_info('Applying room palette override patch...')
            room_filter_override_patch = (Patch('room_filter_override', self.module_base + 0x2e26, self.process).mov_to_eax(room_palette_override_shader).nop(1))
            if room_filter_override_patch.apply():
                self.revertable_patches.append(room_filter_override_patch)
        # endregion
        ...
        #                        .push_r15().push_r14().push_rsi().push_rdi().push_rbp().push_rbx()#.mov_to_rax(address + 0x93670)
        ...
        input_reader_patch = (Patch('input_reader_patch', current_custom_space_offset, self.process)
                              .get_key_pressed(0x48)
                              .cmp_al1_byte(0)
                              .je_near(0x80)
                              .push_rcx().push_rdx().push_r8().push_r9()
                              .warp(address + 0x93670, unstuck_room_x, unstuck_room_y, unstuck_pos_x, unstuck_pos_y, unstuck_map)
        ...
        current_custom_space_offset += len(input_reader_patch)
        ...
        # region Pause Menu Modifications
        """
        ...
        pause_menu_resume_and_warp_patch = (Patch('pause_menu_resume_and_warp_patch', current_custom_space_offset, self.process)
                                            .warp(address + 0x93670, unstuck_room_x, unstuck_room_y, unstuck_pos_x, unstuck_pos_y, 0)
                                            .pop_r9().pop_r8().pop_rdx().pop_rcx()
                                            .mov_to_rax(address)
                                            .jmp_far(0x140044223)
                                            .nop(0x10))

        current_custom_space_offset += len(pause_menu_resume_and_warp_patch)
        ...
        pause_menu_on_confirm_patch = (Patch('pause_menu_on_confirm_patch', current_custom_space_offset, self.process)
                                       .call_far(0x14006ec30)
                                       .push_rcx().push_rdx().push_r8().push_r9()
                                       .mov_from_absolute_address_to_eax(address + 0x93610)
                                       .cmp_eax(2)
        ...
        current_custom_space_offset += len(pause_menu_on_confirm_patch)
        ...
        # region FullBright
        """
        This patch just disables darkness in the game, causing all rooms to be equally lit across all tiles. Mostly useful for debugging.
        """
        fullbright_patch = Patch('fullbright', 0x140102e63, self.process).add_bytes(b'\xeb\x19')

        fullbright_patch_enabled = False

        if fullbright_patch_enabled:
            if self.log_debug_info: self.log_info(f'Applying fullbright patch...\n{fullbright_patch}')
            if fullbright_patch.apply():
                self.revertable_patches.append(fullbright_patch)
        # endregion

        # region Item_Collection_Patches
        """
    ...
    def revert_patches(self):
        ...
        self.custom_memory_space = None  # TODO: Free this memory back when we're unhooking from AW
